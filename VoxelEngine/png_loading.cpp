...
int png_load(const char* path, int* width, int* height) {
    FILE* f;
    ...
    *height = t_height;
    ...
    // Передача данных о текстуре в текстуру
...
Texture* load_texture(std::string path) {
    int width, height;
    GLuint texture = png_load(path.c_str(), &width, &height);
    if (texture == 0) {
        return nullptr;
        std::cerr << "TEXTURE | Unable to load texture from " << path << std::endl;
    }
